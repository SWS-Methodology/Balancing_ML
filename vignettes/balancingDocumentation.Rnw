...
The main function of the package is the {\tt balancing.R}. This function is part of a wrapper in the Standardization module. The balacing function allow to use only normal distribution 


\newpage
\section{Future work}
In Kao's document "Parameterisation of Food Balance Sheet Uncertainty Distribution", additional effort has been made to address several problems: numerical, information
