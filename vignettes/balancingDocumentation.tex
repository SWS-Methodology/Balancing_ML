...
\usepackage{Sweave}
\begin{document}
\input{balancingDocumentation-concordance}
...
%' 
%' <<my-label1, eval=TRUE, echo=F>>=
%' #```{r, echo=FALSE, fig.width=7, fig.height=4, fig=TRUE}
%' temp = faoswsBalancing::balancing(param1 = c(345000, 300000), param2 = c(0, 300000*0.15),
%'           sign = c(1, -1), plot = TRUE, plotSigma = 2)
%' temp = prettyNum(temp, digits = 2, format = "g", big.mark = ",")
%' @
%' 
%' %The balancing algorithm would return the following estimates for production and food, respectively: `r temp`.  The official estimate is not updated, as expected, while the estimate for food is updated to create a balanced equation.
%' 
...
%' temp = faoswsBalancing::balancing(param1 = c(345000, 300000), param2 = c(345000*.1, 300000*0.15),
%'           sign = c(1, -1), plot = TRUE, plotSigma = 2)
%' temp = prettyNum(temp, digits = 2, format = "g", big.mark = ",")
%' @
%' 
%' In this case, the values after balancing for production and food, respectively are `r temp`.  The semi-official estimate is not adjusted as much as the food estimate because it has a lower error.
%' 
